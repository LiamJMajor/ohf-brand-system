...
| Medium | the monthly release post and the changelog entry | long-form, sectioned. Since 2026.9.0 the post opens with a plain-language "Short on time?" summary naming only the changes that affect a typical configuration, then the technical write-up: overview, upgrade checklist, feature sections, contributors, breaking changes, full changelog | accessible: opens from the reader's intention, glosses every term, leads with outcomes. The change-note sections keep the exact config-key register; the surrounding post does not | blog, changelog, Discord, forum | facts cited; contributors credited; the plain-language summary comes first |
