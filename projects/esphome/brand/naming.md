...
- Lowercase `esphome` is correct in exactly three places and nowhere else: the command line tool, the Python package, and repository and URL paths (`esphome/esphome`, `pip install esphome`).
...
## Feature naming
- First-party things are named with ESPHome in front and Title Case after it: ESPHome Device Builder, ESPHome Starter Kit, ESPHome Desktop App. Written in full on first mention in a piece.
